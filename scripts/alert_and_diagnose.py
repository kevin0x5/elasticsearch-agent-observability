...
def _internal_dataset_filter() -> list[dict[str, Any]]:
    """Exclude every ``internal.*`` dataset from alert statistics.

    The ``internal.*`` namespace covers sanity_check, pipeline_verify,
    alert_check, and skill_audit — all self-generated by this skill. Including
    any of them would skew error rate, token sum, or retry baselines in ways
    that look like real alerts but are just the observability plane observing
    itself. A ``prefix`` clause is cheaper than multiple ``term`` clauses and
    picks up new internal datasets automatically.
    """
    ...
    payload = {
        ...
        "query": {
            "bool": {
                ...
                "must_not": _internal_dataset_filter(),
            }
        },
        "aggs": {
            ...
            # analyzer to name the actual hot session. Previously we reused
...
def _analyze_token_anomaly(current: dict[str, Any], baseline: dict[str, Any], multiplier: float) -> dict[str, Any] | None:
    ...
    # hot session only if no token-session data is available (e.g. old
    # ingested docs without session_id). This keeps the RCA honest: the
    # session we name is actually the one burning tokens.
    token_sessions = _extract_value_terms(aggs.get("top_token_sessions", {}), value_agg="token_sum")
    retry_sessions = _extract_value_terms(aggs.get("top_retry_sessions", {}), value_agg="retry_sum")
    if token_sessions:
        primary_session = token_sessions[0]["key"]
        session_source = "token"
    elif retry_sessions:
        primary_session = retry_sessions[0]["key"]
        session_source = "retry_fallback"
    else:
    ...
    return {
        ...
        "evidence": {
            ...
            "top_token_sessions": token_sessions,
            "top_retry_sessions": retry_sessions,
            "primary_session_source": session_source,
        },
    }
...
def _extract_keys(alert: dict[str, Any]) -> dict[str, set[str]]:
    ...
    _add("session", ev.get("top_token_sessions"))
